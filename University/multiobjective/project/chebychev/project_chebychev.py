...
for _ in range(6):
    ...
    m1.update()
    m1.addConstr(w1[_]*(gp.quicksum(x_ij[i, j] * C[i] for i in Set_I for j in Set_J) - ideal1) <= t, name="chebychev1")
    m1.addConstr(w2[_]*(gp.quicksum(x_ij[i, j] * PR[i] for i in Set_I for j in Set_J) - ideal2) <= t, name="chebychev2")
    m1.addConstr(w3[_]*(gp.quicksum(x_ij[i, j] * P[i] for i in Set_I for j in Set_J) - ideal3) <= t, name="chebychev3")
    m1.setObjective(t, GRB.MINIMIZE)
    m1.update()
    m1.write(f"chebychev_solver_{str(_+1)}.lp")
    m1.optimize()
