...
for i in range(6):
    ...
    m1.setObjective(gp.quicksum(x_ij[i, j] * P[i] for i in Set_I for j in Set_J), GRB.MAXIMIZE)
    m1.update()
    m1.write(f"Car_Problem_epsilon_P_{str(i+1)}.lp")
    m1.optimize()
    if m1.status != GRB.Status.INFEASIBLE:
        print("\nCar Problem Solution: (P Obj)", i + 1, m1.objVal)
        for v in m1.getVars():
            if v.x != 0:
                print(v.varName, v.x)
    else:
        print("\nCar Problem Solution: (P Obj)", i + 1, "INFEASIBLE")
