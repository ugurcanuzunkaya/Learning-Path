...
for i in range(6):
    ...
    m1.addConstr(gp.quicksum(x_ij[i, j] * C[i] for i in Set_I for j in Set_J) <= epsilon1, name="Cost constraint")
    m1.addConstr(gp.quicksum(x_ij[i, j] * P[i] for i in Set_I for j in Set_J) <= epsilon2, name="Power constraint")
    m1.update()
    
    m1.setObjective(gp.quicksum(x_ij[i, j] * PR[i] for i in Set_I for j in Set_J), GRB.MINIMIZE)
    m1.update()
    m1.write(f"Car_Problem_epsilon_PR_{str(i+1)}.lp")
    m1.optimize()
    if m1.status != GRB.Status.INFEASIBLE:
        print("\nCar Problem Solution: (PR Obj)", i + 1, m1.objVal)
        for v in m1.getVars():
            if v.x != 0:
                print(v.varName, v.x)
    else:
        print("\nCar Problem Solution: (PR Obj)", i + 1, "INFEASIBLE")
    print()
