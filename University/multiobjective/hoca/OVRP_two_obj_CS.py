...
while True:
    ...
    while True:
        ...
        m1.setObjective((gp.quicksum(distance[i-1, j-1]*x_ijk[i,j,k] for i in set_I for j in set_J for k in set_K)-a1)*w1 + w2 * (beta-a2)
                        +alpha*(abs(gp.quicksum(distance[i-1, j-1]*x_ijk[i,j,k] for i in set_I for j in set_J for k in set_K)-a1)+abs(beta-a2)), GRB.MINIMIZE)
        m1.write('model_obj4.lp')
        m1.optimize()
        ...
        obj[counter][0] = total_distance
        obj[counter][1] = beta.x
        obj[counter][2] = alpha
        print('******************************')
        print(m1.objVal)
        alpha +=1
