...
while True:
    ...
    m1.addConstr(a1 + t*r1 -(gp.quicksum(distance[i-1, j-1]*x_ijk[i,j,k] for i in set_I for j in set_J for k in set_K))>=0 , name='pss1')
    m1.addConstr((a2 + t*r2- beta >=0), name='pss2')
    m1.setObjective(t, GRB.MINIMIZE)
    m1.write('model_obj6.lp')
    m1.optimize()
    ...
    if m1.status not in [GRB.INFEASIBLE, GRB.UNBOUNDED, GRB.INF_OR_UNBD]:
        ...
        obj[sayac - 1][0] = total_distance
        obj[sayac - 1][1] = beta.x
        obj[sayac - 1][2] = r1
        obj[sayac - 1][3] = r2
        print('******************************')
        print(m1.objVal)
